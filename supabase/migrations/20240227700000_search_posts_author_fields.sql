-- search_posts: return author anon_fingerprint and country_code for UI badge
-- Must drop first because return type (OUT parameters) is changing.

drop function if exists search_posts(text, text, integer, integer);
...
begin
  ...
  if p_query = '' then
    ...
    group by p.id, p.title, p.content, p.created_at, p.anon_fingerprint, p.country_code, c.slug
